...
class StockManagementApp:
    def __init__(self, window):
        self.window = window
        self.window.title("Stock Management App")
        self.create_widgets()
...
def save():
    ...
    if not(itemId and itemId.strip()) or not(name and name.strip()) or not(price and price.strip()) or not(qnt and qnt.strip()) or not(cat and cat.strip()):
        ...
        return
    if len(itemId) < 5:
        messagebox.showwarning("","Id Barang Tidak Valid")
        return
    if(not(itemId[3]=='-')):
        valid=False
    for i in range(0,3):
    ...
    try:
        ...
        if len(checkItemNo) > 0:
            messagebox.showwarning("","Id Item sudah digunakan")
            return
        else:
            cursor.connection.ping()
...
def delete():
    try:
        if(my_tree.selection()[0]):
            decision = messagebox.askquestion("", "Hapus data yang dipilih?")
            if(decision != 'yes'):
                return
            else:
                selectedItem = my_tree.selection()[0]
                ...
                try:
                    ...
                    conn.commit()
                    conn.close()
                    messagebox.showinfo("","Data telah berhasil dihapus")
                except:
...
def find():
    ...
    if(itemId and itemId.strip()):
        ...
        messagebox.showwarning("","Silakan isi salah satu entri")
        return
    cursor.execute(sql)
    try:
        ...
        messagebox.showwarning("","Tidak ada data ditemuka")
...
def exportExcel():
    ...
    conn.commit()
    conn.close()
    messagebox.showinfo("","File Excel diunduh")
...
saveBtn=Button(manageFrame,text="SIMPAN",width=10,borderwidth=3,bg=btnColor,fg='white',command=save)
updateBtn=Button(manageFrame,text="PERBAHARUI",width=10,borderwidth=3,bg=btnColor,fg='white',command=update)
deleteBtn=Button(manageFrame,text="HAPUS",width=10,borderwidth=3,bg=btnColor,fg='white',command=delete)
selectBtn=Button(manageFrame,text="PILIH",width=10,borderwidth=3,bg=btnColor,fg='white',command=select)
findBtn=Button(manageFrame,text="MENEMUKAN",width=10,borderwidth=3,bg=btnColor,fg='white',command=find)
clearBtn=Button(manageFrame,text="BERSIHKAN",width=10,borderwidth=3,bg=btnColor,fg='white',command=clear)
exportBtn=Button(manageFrame,text="EXPOR EXCEL",width=15,borderwidth=3,bg=btnColor,fg='white',command=exportExcel)
...
itemIdLabel=Label(entriesFrame,text="IDENTITAS",anchor="e",width=10)
nameLabel=Label(entriesFrame,text="NAMA",anchor="e",width=10)
priceLabel=Label(entriesFrame,text="HARGA",anchor="e",width=10)
qntLabel=Label(entriesFrame,text="KUANTITAS",anchor="e",width=10)
categoryLabel=Label(entriesFrame,text="KATEGORI",anchor="e",width=10)
...
categoryArray=['Alat Jaringan','BAGIAN KOMPUTER','Alat Perbaikan','gadget']
...
generateIdBtn=Button(entriesFrame,text="MEMBUAT ID",borderwidth=3,bg=btnColor,fg='white',command=generateRand)
...
my_tree.heading("Item Id",text="Identitas",anchor=W)
my_tree.heading("Name",text="Nama",anchor=W)
my_tree.heading("Price",text="harga",anchor=W)
my_tree.heading("Quantity",text="Kuantitas",anchor=W)
my_tree.heading("Category",text="Kategori",anchor=W)
my_tree.heading("Date",text="Tanggal & waktu",anchor=W)
my_tree.tag_configure('orow',background="#EEEEEE")
my_tree.pack()
