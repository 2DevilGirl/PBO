...
def login(username, password, label_result, root, frame_login, frame_registration):
    ...
    if connection:
        try:
            ...
            if result:
                label_result.config(text='Login berhasil.', foreground='green')
                cursor.close()
                connection.close()
                on_login_success(root)
                return
            else:
                label_result.config(text='Akun email belum terdaftar.', foreground='red')

        except pymysql.Error as err:
            label_result.config(text=f'Error: {err}', foreground='red')
        finally:
            cursor.close()
            connection.close()
...
def simpan_data_login(username, password, label_result, frame_login, frame_registration):
    ...
    if connection:
        try:
            ...
            cursor.execute(query, (username, password))
            connection.commit()
            label_result.config(text='Registrasi berhasil. Data login disimpan.', foreground='green')
...
def main():
    ...
    style.set_theme("equilux")  # Change the theme to 'equilux'
    ...
    style.configure("TButton", foreground="white", background="#2196F3", font=('Arial', 12, 'bold'))
